...
import {
  ...
  Plus, Loader2, Save, Printer
} from 'lucide-react'

// ── Print invoice ─────────────────────────────────────────────────────────────
function printInvoice(invoice, clinicName) {
          ...
          {visit.inv ? (
            <div style={{ marginBottom: 12 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 5 }}>
                <p style={{ fontSize: 10, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', color: '#94a3b8', margin: 0 }}>Invoice</p>
                <button
                  onClick={() => printInvoice(visit.inv, null)}
                  style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 11, color: '#64748b', background: '#f1f5f9', border: 'none', borderRadius: 6, padding: '3px 8px', cursor: 'pointer', fontWeight: 600 }}
                >
              ...
              <div style={{ background: '#fff', border: '0.5px solid #e2e8f0', borderRadius: 8, padding: '9px 12px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div>
                  ...
                  {((visit.inv.total || 0) - (visit.inv.paid_amount || 0)) > 0 && (
                    <p style={{ fontSize: 11, color: '#b91c1c', margin: '4px 0 0', fontWeight: 600 }}>
                      Due: ৳{((visit.inv.total || 0) - (visit.inv.paid_amount || 0)).toLocaleString()}
                    </p>
                  )}
                </div>
                <span style={{
