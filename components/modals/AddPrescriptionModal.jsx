...
// Hover/focus dropdown — no button, options appear on hover or focus
  ...
  return (
    <div className="border border-slate-200 rounded-xl p-3 space-y-2">
      <div className="flex items-center justify-between mb-1">
        <span className="text-xs font-semibold text-emerald-600">Medicine {index + 1}</span>
      ...
      <div className="grid grid-cols-3 gap-2">
        <HoverDropdown value={med.frequency} options={FREQ_OPTIONS}
          onChange={val => onChange(index, 'frequency', val)} placeholder="Frequency" />
        <HoverDropdown value={med.duration} options={DUR_OPTIONS}
          onChange={val => onChange(index, 'duration', val)} placeholder="Duration" />
        <HoverDropdown value={med.instructions} options={INSTR_OPTIONS}
          onChange={val => onChange(index, 'instructions', val)} placeholder="Instructions" />
      </div>
    </div>
  )
}
